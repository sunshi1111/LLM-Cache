...
def batch_test(model, device, output_len=50, debug=False):
  """运行预设批量测试对话，保留对话历史"""
  print("\n===== 开始批量测试对话 =====")
  reset_timer()
  ...
  for idx, user_input in enumerate(test_inputs):
    ...
    current_prompt += format_message("user", user_input)
    current_prompt += "<start_of_turn>model"
    ...
    results.append({
      ...
      "second_loop_time": TOTAL_SECOND_LOOP_DURATION if debug else 0
    })
    
    # 更新对话历史
    # 去掉conversation_history中已有的"<start_of_turn>model"
    current_prompt = current_prompt.replace("<start_of_turn>model", "")
    conversation_history = current_prompt + format_message("model", model_response)
