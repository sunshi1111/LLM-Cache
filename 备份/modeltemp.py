...
class GemmaForCausalLM(nn.Module):
    ...
    def generate(
            ...
            start_pos = 0
            if kv_caches is not None and not force_regenerate:
    # 获取已缓存的有效长度
                cached_length = kv_cache_metadata.get("original_prompt_len", 0)
                print(f"│ - 已缓存长度: {cached_length}，将从此处继续生成          │")
    
    # 验证缓存有效性
                if cached_length > 0 and cached_length <= min_prompt_len:
        # 设置起始位置为已缓存的长度
                    start_pos = cached_length
        # 调整output_index以从缓存结束处开始
                    output_index = torch.tensor(cached_length, dtype=torch.int64).to(device)
                    print(f"│ - 利用KV cache，从位置{start_pos}开始生成              │")
                else:
                    print("│ - 缓存长度无效或不适用于当前输入                     │")
            ...
            for i in range(start_pos, max_seq_len - min_prompt_len):
    # 明确指定当前位置，避免覆盖已有缓存
                curr_pos = torch.tensor([i], dtype=torch.int64).to(device)
    
                next_token_ids, _ = self(
                    input_token_ids=input_token_ids_tensor,
                    input_positions=input_positions_tensor,
                    kv_write_indices=curr_pos,  # 关键修改：指定写入位置
                    kv_caches=kv_caches,
